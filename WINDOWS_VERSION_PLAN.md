...
**注意**: 開発・検証は`takan`（開発機、管理者）と`matht`（同じ開発機上のテスト用標準ユーザー）の2アカウントで行っているが、実際の対象端末は子供が使う**別の物理ノートPC**（このリポジトリを開発している端末とは別）。`takan`/`matht`はあくまで「全アカウントで保護」機能の複数アカウント動作を検証するためのスタンドインであり、最終的にはその別のノートPCに実機インストールする必要がある（本日以降のタスク）。

### ステータス
**ベータ版 (Beta 1.0) 完全完成・実機テスト検証完了済み（同一開発機上の複数アカウント間）**。
- ビルド確認: 0エラー・0警告
- 実機動作検証: 制限ブロック画面、保護者パスワード解錠、使い捨て一時パスワード、勉強時間帯制限バイパス、全アカウント保護（HKLM自動起動・ProgramData権限・icacls ACL削除防止、UAC昇格を伴う実際の有効化フロー、サインアウトでは反映されずPC再起動が必要という挙動、他アカウントの既起動プロセスへの設定反映）、二重起動時のダッシュボード最前面表示、デスクトップショートカット等、すべて動作検証完了。
- **未検証**: 子供が実際に使う別のノートPCへの実機インストール。Chrome Remote Desktopが制限モード中も実際に機能するかの確認。
...
- **UAC昇格を伴う「全アカウントで保護」の実際の有効化フロー**（2026-07-27〜08-01、`takan`(管理者)/`matht`(標準ユーザー)の2アカウントで検証済み）
- 有効化後、PC再起動を経て`matht`側でも自動起動・制限が実際にかかることを確認済み（サインアウト→サインインのみでは不十分、PC再起動が必要という制約込み）
- `takan`側の設定変更（勉強時間・上限分等）が、既に起動している`matht`側のプロセスにも数秒以内に反映されること（2026-08-01の修正後、実機確認済み）

### 未検証（次にやるべきこと・実機での確認が必要）
- **子供が実際に使う別のノートPCへの実機インストール**（これまでの検証はすべて開発機上の複数アカウントによるスタンドインテスト）
...
### 子供のPCへの実機インストール手順
これまでの検証はすべて開発機（`takan`/`matht`の2アカウント）上で行ってきたが、実際に保護対象となるのは**子供が使う別のノートPC**。そちらへのインストールは未実施・未検証。手順:

1. **事前確認（対象PC）**
   - Windows 10/11であること
   - `.NET 8 Desktop Runtime (x64)` が入っているか確認（無ければ https://dotnet.microsoft.com/download/dotnet/8.0 のDesktop Runtime x64を導入。管理者権限が必要）。両csprojに`RollForward=LatestMajor`を設定済みなので、8.0ちょうどが無くても9/10系のRuntimeがあれば動く
   - WebView2ランタイムは最近のWindows 10/11ならEdgeと共に標準搭載済みのことが多いが、念のため確認（無ければ https://developer.microsoft.com/microsoft-edge/webview2/ から「Evergreen Bootstrapper」を導入）

2. **アプリ本体の配置**
   - 開発機で最新の`main`ブランチをビルド・publish済みのフォルダ（`WindowsApp/publish/framework-dependent/`、本体・Watchdog両方の実行ファイル一式）をUSBメモリ等でコピーするか、対象PCに直接.NET 8 SDKを導入して`git clone`→`dotnet publish`する
   - 配置先は任意だが、`C:\Users\<保護者アカウント>\Documents\MedicalSchoolApp.Windows\`のような分かりやすい場所を推奨（後で「全アカウントで保護」を有効化すると`%ProgramData%\MedicalSchoolApp.Windows\bin\`へ自動的にコピーされるので、この配置場所自体は一時的なものでよい）

3. **初回起動・初期設定（保護者の管理者アカウントで）**
   - `MedicalSchoolApp.Windows.exe`を実行 → ダッシュボードが起動する
   - 保護者設定を開く（初期パスワード`0000`）→ **すぐにパスワードを変更する**
   - 1日の使用可能時間（デフォルト90分）、曜日別勉強時間、許可アプリリスト、共通テスト日を子供の実情に合わせて設定
   - デスクトップショートカットを作成（`explorer shell:startup`直下ではなく通常のデスクトップに、対象exeへのショートカットを配置。手動で右クリック→ショートカットの作成、で十分）

4. **全アカウントで保護を有効化**
   - 保護者設定の「全アカウントで保護」ボタンを押す → UAC昇格ダイアログで管理者パスワードを入力
   - 有効化後は**サインアウト→サインインだけでは反映されないことがあるため、PCを一度完全に再起動する**
   - 再起動後、子供のアカウントでログインし、トレイアイコン（`^`隠れているアイコンの中にある場合あり）とダッシュボードが自動的に起動していることを確認

5. **最終確認**
   - 子供のアカウントで、許可リストに無いアプリ（ゲーム等）を開き、勉強時間中または上限時間超過後にブロック画面が出て閉じられることを確認
   - ブロック画面からChatGPT画面が開けることを確認
   - タスクマネージャーで本体またはWatchdogを強制終了しても、もう一方が数秒で再起動させることを確認
   - Chrome Remote Desktopが導入済みなら、制限モード中でも保護者が別端末からリモート接続できることを確認（**未検証項目**、実際に試すこと）
   - 保護者パスワードを`0000`から変更済みであることを再確認
...
### 追記（2026-07-27）: 全アカウントでの保護が実際には効いていなかった不具合を修正
実機で「全アカウントで保護」を有効化してWindowsアカウントを切り替えたところ、全く動作しないという報告があり、原因を特定して修正した。

**原因**: `MachineWideSetupService.RunElevatedRegistration()`が、HKLMのRunキーに登録するexeのパスとして`AppContext.BaseDirectory`（＝そのとき実行していた場所、例えば特定アカウントの`Documents`配下）をそのまま使っていた。Windowsでは他のユーザーアカウントは他人の`C:\Users\<アカウント名>\`配下に既定でアクセス権が無いため、別アカウントでログインしてもそのパスのexeを実行できず、自動起動が一切機能していなかった。

**修正内容**:
- `RunElevatedRegistration()`は、登録前にexe本体一式（依存DLL・ネイティブファイル含む全ファイル）を`%ProgramData%\MedicalSchoolApp.Windows\bin\`へ再帰的にコピーするようにした。このフォルダには`ProgramDataDir`本体と同じ`icacls`によるUsers権限の継承付与が及ぶため、どのアカウントからも実行できる。HKLM Runキーにはコピー後のこの`bin`配下のパスを登録する。
- `IsMachineWideEnabled`の判定を、`ProgramDataフォルダの存在`から**実際のHKLM Runキーの存在**（読み取りのみ、管理者権限不要）に変更した。従来は無効化してもフォルダ（設定・履歴用に残す方針）が残っているため「有効」表示のままになり、再登録（無効化→有効化のやり直し）ができなかった。

**この修正を反映するには**: 既に古い（`bin`コピーを伴わない）登録をしてしまっている場合、保護者設定で一度「このアカウント専用に戻す」→「全アカウントで保護を有効にする」を実行し直す必要がある（登録し直しにより`%ProgramData%\MedicalSchoolApp.Windows\bin\`にコピーされたexeがHKLMに登録される）。

**実機検証済み（2026-07-27）**: `matht`（標準ユーザー）アカウントで、上記の登録し直し後に確認したところ、**サインアウト→サインインだけでは自動起動されず、PC自体を完全に再起動して初めて自動起動された**。この環境ではHKLMのRunキーの新規追加が、既存セッションへの再ログインでは反映されず、システム全体の再起動で初めて認識される挙動だった模様。

→ **「全アカウントで保護」を有効化（または再登録）した後は、サインアウト/サインインではなく必ずPC自体を再起動するよう案内すること。** 保護者設定のメッセージ文言に反映済み。

### 追記（2026-08-01）: 別アカウントで「トレイアイコンが出ずダッシュボードを閉じるとアプリが消えたように見える」不具合を修正
上記の再起動後、`matht`で自動起動はしたが、トレイアイコンが表示されず、ダッシュボードの✖を押すと何も残らず「アプリが消えた」ように見えるという報告があった。

**調査結果**: `crash.log`（新設）が生成されず、`Get-Process`でも本体プロセスは生きたまま確認できたことから、**実際にはクラッシュしておらず、単にトレイアイコンの登録がそもそも失敗していた**ことが判明。自動起動（Runキー経由の起動）はWindowsのタスクバー（通知領域）の初期化が完了する前に走ることがあり、その場合`NotifyIcon`の登録が silently 失敗し、二度と表示されないことがある（自動起動アプリでよくある既知の問題）。ダッシュボードが唯一の目に見える手がかりだったため、それを閉じる（実際にはHideしているだけ）と、外からは何も操作できるものが無くなり「アプリが消えた」ように見えていた。

**修正内容**（`App.xaml.cs`）:
- Explorerがタスクバーを(再)作成した時に送る`TaskbarCreated`メッセージを`RegisterWindowMessage`で監視し、受信したらトレイアイコンを作り直すようにした（`HwndSource.AddHook`でダッシュボードウィンドウのウィンドウプロシージャにフックする、tray常駐アプリの標準的な対策）。
- `SetupTrayIcon()`を複数回呼んでも安全なように、既存アイコンを破棄してから作り直す形にした。
- 副次的に、`OnStartup`全体を`try/catch`で囲み、`AppDomain.UnhandledException`・`DispatcherUnhandledException`・監視タイマー内の例外もすべて`%ProgramData%(または%APPDATA%)\MedicalSchoolApp.Windows\crash.log`に記録するようにした（今後同種の問題が起きた際に原因究明しやすくするため）。

**真の原因が判明（2026-08-01）**: 上記の修正後も改善しなかったため詳しく確認したところ、**トレイアイコンは実際には最初から正常に表示されていた**。Windowsは新しく現れたトレイアイコンを既定で「隠れているアイコン」（タスクバー時計付近の`^`をクリックすると出てくる場所）に自動的に格納する仕様があり、それに気づいていなかっただけだった。つまりアプリ自体には元々バグは無く、UIの見つけにくさが原因だった。

`TaskbarCreated`対応・`crash.log`記録は無駄ではなく、実際の不具合が今後発生した際の備えとして残す。実運用では、保護者に「初回は`^`（隠れているアイコン）の中を確認してください」と案内するか、タスクバーの設定でこのアプリのアイコンを常時表示に変更してもらう（`設定 → 個人用設定 → タスクバー → その他のシステムトレイアイコン`）ことを推奨する。

**未検証**: 修正後、実機で実際にトレイアイコンが表示されるようになったかは未確認（このセッションのツールからは検証できない）。次に別アカウントで自動起動を試した際、トレイアイコンの有無を確認する必要がある。

### 追記（2026-08-01 その2）: 全アカウント保護モードで、他アカウントの設定変更が実行中プロセスに反映されない不具合を修正
トレイアイコンの問題解消後、新たな報告があった: `takan`側で現在時刻を含む勉強時間帯を設定すると`takan`側では即座に制限がかかるが、その状態のまま`matht`に切り替えても`matht`側は無制限のまま使えてしまう（ダッシュボードは起動しているが機能していないように見える）。

**原因**: `AppState.Settings`・`AppState.Usage`は静的プロパティの初期化子（`= SettingsService.Load()` / `= UsageService.Load()`）でプロセス起動時に一度だけ読み込まれ、それ以降ディスクから読み直す処理が一切無かった。「全アカウントで保護」時は複数アカウントの本体プロセスが同じ`%ProgramData%\MedicalSchoolApp.Windows\settings.json`を共有するが、既に起動済みのプロセスは他アカウントが書き換えた内容をメモリ上のキャッシュとして持ち続けたまま反映しない設計になっていた。

**修正内容**:
- `Services/AppState.cs`に`ReloadSettings()`（`Settings`をディスクから再読込）・`ReloadUsage()`（`Usage`をディスクから再読込）を追加。
- `Services/ProcessMonitor.cs`の`Tick()`（1秒ごとに呼ばれる監視ループ）の先頭で毎回`AppState.ReloadSettings()`を呼ぶようにした。Settingsはこのプロセス自身が随時書き足すものではない（保護者設定画面での保存時のみ書き込み）ため、常に読み直しても自分の変更を失う心配が無い。
- `Usage`は1秒ごとの使用時間加算をメモリ上に溜めて5秒毎に書き出す方式のため、書き出し前に読み直すと自分自身の未保存の増分を失ってしまう。そのため`AppState.SaveUsage()`の**直後**（書き出し完了後）にのみ`AppState.ReloadUsage()`を呼ぶようにした（5秒に1回、`Tick()`内の`if (now.Second % 5 == 0)`ブロック内）。

この変更により、`ParentSettingsWindow`や`DashboardWindow`など`AppState.Settings`/`AppState.TodayUsage`を直接参照している箇所は、`Tick()`が動いている限り最大1秒（Usageは最大5秒）の遅延で他アカウントの変更を自動的に反映するようになった。

**ビルド・配布**: `dotnet build`成功（0エラー・0警告）。`dotnet publish -c Release`で本体・Watchdog双方を再パブリッシュし、`%ProgramData%\MedicalSchoolApp.Windows\bin\`へ上書きコピー済み（`takan`側のプロセスは`taskkill`で一旦終了させてから実施。`matht`側は権限上`taskkill`できないが、実行中でもexe/dllファイル自体の上書きは可能なため問題なし。次回`matht`側プロセスが再起動されるか、Watchdogの相互監視で再起動された際に新しいバイナリが使われる）。

**未検証**: 実機での再現テスト（`takan`で現在時刻を含む勉強時間を設定→`matht`に切り替え→数秒以内に制限がかかることを確認）はまだ行われていない。次にユーザーに依頼する。
